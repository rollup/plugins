...
describe('rollup-plugin-babel when used as an input plugin', () => {
  ...
  it('runs code through babel', async () => {
    const { code } = await generate('samples/basic/main.js');
...
`
  ...
  it('generates sourcemap by default', async () => {
    const { code, map } = await generate('samples/class/main.js', {}, { sourcemap: true });
    const target = 'log';
    ...
    assert.deepEqual(original, {
      source: 'samples/class/main.js'.split(nodePath.sep).join('/'),
      line: 3,
      column: 10,
      name: target
    });
  ...
  it('allows transform-runtime to be used instead of bundled helpers', async () => {
    ...
    assert.deepStrictEqual(warnings, [
      `'@babel/runtime/helpers/classCallCheck' is imported by samples${nodePath.sep}runtime-helpers${nodePath.sep}main.js, but could not be resolved – treating it as an external dependency`
    ]);
    assert.strictEqual(
      code,
      `'use strict';
...
`
  ...
  it('allows transform-runtime to inject esm version of helpers', async () => {
    ...
    assert.deepStrictEqual(warnings, [
      `'@babel/runtime/helpers/esm/classCallCheck' is imported by samples${nodePath.sep}runtime-helpers-esm${nodePath.sep}main.js, but could not be resolved – treating it as an external dependency`
    ]);
    assert.strictEqual(
      code,
...
`
  ...
  it('allows transform-runtime to be used instead of bundled helpers, but throws when CommonJS is used', async () => {
    try {
      await generate('samples/runtime-helpers-commonjs/main.js', { babelHelpers: 'runtime' });
      assert.ok(false);
    } catch (error) {
      assert.ok(
  ...
  it('allows using external-helpers plugin in combination with @babel/plugin-external-helpers', async () => {
    const { code } = await generate('samples/external-helpers/main.js', {
...
`
  ...
  it('correctly renames helpers (#22)', async () => {
    const { code } = await generate('samples/named-function-helper/main.js');
    assert.ok(!code.includes('babelHelpers_get get'), 'helper was incorrectly renamed');
  });
  ...
  it('transpiles only files with whitelisted extensions', async () => {
    const { code } = await generate('samples/extensions-custom/main.js', {
      extensions: ['.js', '.other']
    });
  ...
  it('throws when trying to add babel helper unavailable in used @babel/core version', async () => {
    try {
      await generate('samples/basic/main.js', {
        plugins: [
          function testPlugin() {
            return {
              visitor: {
  ...
  it('works with minified bundled helpers', async () => {
    ...
    await generate('samples/class/main.js', {
      plugins: [
        function testPlugin({ types: t }) {
          return {
            visitor: {
              FunctionDeclaration(path) {
                ...
                path
                  .get('id')
                  .replaceWith(t.identifier(String.fromCharCode(BASE_CHAR_CODE + counter)));
...
describe('rollup-plugin-babel when used as an output plugin', () => {
  async function generateWithOutputPlugin(
    input,
    babelOptions = {},
    generateOptions = {},
    rollupOptions = {}
  ...
  it('allows running the plugin on the output via output options', async () => {
    const { code } = await generateWithOutputPlugin('samples/basic/main.js', {
      presets: ['@babel/env']
    });
    assert.ok(code.indexOf('const') === -1, code);
  });
  ...
  it("allows transform-runtime to be used with `useESModules: false` (the default) and `format: 'cjs'`", async () => {
    const { code } = await generateWithOutputPlugin(
      'samples/runtime-helpers/main.js',
      {
        presets: ['@babel/env'],
...
`
  ...
  it('generates sourcemap by default', async () => {
    const { code, map } = await generateWithOutputPlugin(
      'samples/class/main.js',
      {},
      { sourcemap: true }
    );
    const target = 'log';
...
`
  ...
  it('warns when using the "include" option', async () => {
    const warnings = [];
    await generateWithOutputPlugin(
...
`
  ...
  it('throws when using a Rollup output format other than esm or cjs', async () => {
    try {
      await generateWithOutputPlugin('samples/basic/main.js', {}, { format: 'iife' });
...
`
  ...
  it('loads configuration files when configFile is passed', async () => {
    const { code } = await generateWithOutputPlugin('samples/config-file/main.js', {
      configFile: nodePath.resolve(__dirname, 'samples/config-file/config.json')
    });
    assert.strictEqual(
      code,
      `'use strict';
